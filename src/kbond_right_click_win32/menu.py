...
⚠️ 알려진 문제: 별도 스레드에서 호출 시 메뉴가 즉시 닫힘 (cmd=0 반환)
"""
...
def show_custom_menu(x, y, sentence_text, all_text):
    ...
    try:
        # 현재 포그라운드 창을 소유자로 사용 (정석적인 방법)
        hwnd_owner = win32gui.GetForegroundWindow()
        print(f"[{ts()}] Using foreground window as owner: {hwnd_owner}")
